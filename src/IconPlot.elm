...
stickfigureplot model =
 -- funktionen und parameter deklarieren
    let
        ...
        uValues : List Float
        uValues =
            List.map .u model.data --u

        vValues : List Float
        vValues =
            List.map .v model.data --v

        pValues : List Float
        pValues =
            List.map .p model.data --p

        qValues : List Float
        qValues =
            List.map .q model.data --q
        ...
        labelPositions =
            { x = wideExtent xValues |> Tuple.second
    ...
    -- plot x axis    
         , g[ transform [ Translate (60) (390)]]
            [
                xAxis xValues
                , text_
                [ x ( Scale.convert xScaleLocal (labelPositions.x - 5) )
                , y 35
...
type alias Point =
    { pointName : String, x : Float, y : Float, u : Float, v : Float , p : Float , q : Float , z : Float , w : CarType }
...
getName: String -> Maybe Int -> Maybe Int -> Maybe Int -> Maybe Int -> Maybe Int -> Maybe Int -> Maybe Float -> CarType -> Point
getName n m p d l we wi es t= 
    let
        f x = Maybe.withDefault (0) (x)
    in 
        Point ((n) ++ " (" ++ String.fromInt (f m) ++ ", " ++ String.fromInt (f p) ++ ")") (toFloat (f m)) (toFloat (f p)) (toFloat (f d)) (toFloat (f l)) (toFloat (f we)) (toFloat (f wi)) (f es) (t)

carToPoint: Car -> Point
carToPoint car = 
    getName  car.vehicleName car.cityMPG car.retailPrice car.dealerCost car.carLen car.weight car.carWidth car.engineSize car.carType

filterAndReduceCars : List Car -> XyData
filterAndReduceCars my_cars =
    
    XyData "cityMPG" "retailPrice" "dealerCost" "carLen" "weight" "carWidth" "engineSize" "carType"  (List.map carToPoint (filter my_cars))

filter: List Car -> List Car
filter listcar = 
    let
        p car = car.cityMPG /= Nothing && car.retailPrice /= Nothing && car.dealerCost /= Nothing && car.carLen /= Nothing && car.weight /= Nothing && car.carWidth /= Nothing && car.engineSize /= Nothing
    in 
        List.filter p listcar
